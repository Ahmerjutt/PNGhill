        ...
        <div class="col s12 m5 l4">
            <div class="mainDownlaod">
              ...
              <?php $zi = pathinfo($value->dlink_zip) ?>
                <div class="downbtns">
                    <a target="_blank" href="<?=base_url('download?go='.$pngcode.'&t='.time().'_'.$value->ID.'&ex='.$pi['extension'])?>" class="waves-effect waves-brown btn lightbtn"><i class="material-icons left">cloud_download</i>Download <?=ltrim($Info->file_ext, '.')?></a>
                </div>
                <?php if ($value->dlink_zip != ''): ?>
                  <?php if ($controller->isUser()): ?>
                    <div class="downbtns">
                        <a target="_blank" href="<?=base_url('download?go='.$zipcode.'&t='.time().'_'.$value->ID.'&ex=zip')?>" class="waves-effect waves-teal btn lightbtn lbbtn"><i class="material-icons left">cloud_download</i>Download <?=$value->workWith?></a>
                    </div>
                  <?php else: ?>
                    <div class="downbtns">

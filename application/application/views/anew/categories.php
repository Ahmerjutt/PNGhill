          ...
          <?php foreach ($cats->result() as $key => $value): ?>
            <div class="m-0 mb-0 pb-0">
              <div class="flex p-4 text-center bg-white rounded-md dark:bg-darker shadow hover:shadow-lg justify-between">
                <div class="flex">
                   <a href="" class="text-gray-600 fontt hover:text-primary-50 hover:underline"><?=$value->cname?></a>
                   <a href="" class="bg-blue-50 shadow-sm p-1 px-2 rounded text-blue-600 text-xs ml-2">4</a>
                 </div>
                <div class="">
                   <a href="<?=base_url('admin-panel/add/category?edit='.$value->CID)?>" class="text-primary-lighter font-bold py-1 px-3 rounded text-xs bg-primary-50 hover:bg-blue-100">Edit</a>
                   <a href="<?=base_url('admin-panel/edit?action=category&task=delete&id='.$value->CID)?>" class="text-red-400 font-bold py-1 px-3 rounded text-xs bg-red-50 hover:bg-red-100">Delete</a>
                 </div>
              </div>
            </div>
          <?php endforeach; ?>
